...
HERE = ROOT / "campaigns" / "odd_zeta" / "OZ_RT_BZ_T3_011_K"
...
class OzRtBzT3011KTests(unittest.TestCase):
    ...
    def test_exact_tridegree_solver_has_no_cutoff(self) -> None:
        base = ()
        left = (("L", 1),)
        right = (("R", 1),)
        spectator = (("S", 1),)
        target = (("L", 17), ("R", 23), ("S", 31))
        self.assertEqual(
            producer._solve_tridegrees(base, left, right, spectator, target),
            ((17, 23, 31),),
        )
    ...
    def test_producer_and_independent_verifier(self) -> None:
        ...
        if result["terminal"] == producer.ESCAPE_TERMINAL:
            ...
            self.assertFalse(
                result["all_trivariate_spectator_responses_cokernel_invisible"]
            )
            ...
            + json.dumps(
                {
                    ...
                    "all_trivariate_spectator_responses_cokernel_invisible": result[
