...
HERE = ROOT / "campaigns" / "odd_zeta" / "OZ_RT_BZ_T3_011_H"
...
class OzRtBzT3011HTests(unittest.TestCase):
    def test_scope_rejects_widening(self) -> None:
        for kwargs in (
            {"shifted_poles": True},
            {"mixed_reciprocal_products": True},
    ...
    def test_producer_and_independent_verifier(self) -> None:
        ...
        if result["terminal"] == producer.ESCAPE_TERMINAL:
            ...
            self.assertFalse(result["all_reciprocal_axis_responses_cokernel_invisible"])
            ...
            self.assertTrue(result["all_reciprocal_axis_responses_cokernel_invisible"])
            ...
            + json.dumps(
                {
                    ...
                    "all_reciprocal_axis_responses_cokernel_invisible": result[
