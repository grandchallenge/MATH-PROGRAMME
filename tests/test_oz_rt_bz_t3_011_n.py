...
HERE = ROOT / "campaigns" / "odd_zeta" / "OZ_RT_BZ_T3_011_N"
...
class OzRtBzT3011NTests(unittest.TestCase):
    def test_contract_binds_scope_and_predecessor(self) -> None:
        contract = json.loads((HERE / "CONTRACT.json").read_text(encoding="utf-8"))
        self.assertEqual(contract["issue"], 929)
        self.assertEqual(contract["operation"], producer.OPERATION)
        self.assertEqual(contract["predecessor"]["reviewed_head"], producer.M_REVIEWED_HEAD)
        self.assertEqual(contract["predecessor"]["merge_commit"], producer.M_MERGE_COMMIT)
        self.assertEqual(contract["predecessor"]["required_terminal"], producer.M_REQUIRED_TERMINAL)
        scope = contract["double_reciprocal_active_trivariate_class"]
        self.assertEqual(scope["reciprocal_active_axis_count"], 2)
        self.assertFalse(scope["reciprocal_spectator_authorized"])
        self.assertFalse(scope["active_plus_spectator_double_reciprocal_authorized"])
    ...
    def test_producer_and_independent_verifier_agree_on_exact_terminal(self) -> None:
        ...
        if result["terminal"] == producer.CLOSURE_TERMINAL:
            ...
            self.assertTrue(result[
                "all_double_reciprocal_active_trivariate_responses_cokernel_invisible"
            ])
            ...
            + json.dumps(
                {
                    ...
                    "all_double_reciprocal_active_trivariate_responses_cokernel_invisible": result[
                        "all_double_reciprocal_active_trivariate_responses_cokernel_invisible"
                    ],
                },
                sort_keys=True,
