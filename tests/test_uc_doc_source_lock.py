...
class UnionClosedDocumentarySourceLockTests(unittest.TestCase):
    ...
    def test_work_package_bundle_is_complete(self) -> None:
        required = ["00_README.md", "01_RESULT_STATUS.json", "02_LAY_COMPANION.md", "03_OBJECT_AND_OBSTRUCTION.md", "04_PROBLEM_AND_STATUS_AUDIT.md", "05_THEOREM_SPINE.md", "06_DEPENDENCY_DAG.json", "07_PROOFS_AND_COMPUTATIONS.md", "08_FAILURE_AND_NEGATIVE_RESULTS.md", "09_PROOF_DEBT.json", "10_CLAIM_LEDGER.yaml", "11_CERT_HANDOFF.md", "12_NEXT_EXECUTABLE_STEP.md", "artifacts/UC-DOC-WP00_SOURCE_LOCK.json"]
        for relative in required:
            self.assertTrue((WP00 / relative).is_file(), relative)
        for relative in ("00_README.md", "01_RESULT_STATUS.json", "artifacts/UC-DOC-WP01_ADMISSION.json"):
            self.assertTrue((WP01 / relative).is_file(), relative)

    def test_release_identities_match_source_record(self) -> None:
        for key, label in (("latex_source", "Complete LaTeX source"), ("rendered_pdf", "Rendered PDF"), ("authoritative_source_bundle", "Authoritative complete illustrated source bundle")):
    ...
    def test_deferred_source_lock_is_discharged_atomically(self) -> None:
        ...
        volume = next(v for v in self.manifest["volumes"] if v["slug"] == "union_closed")
        self.assertEqual("union_closed.edition.json", volume["edition_record"])
        self.assertEqual("union_closed.md", volume["web_page"])
    ...
    def test_source_record_fields(self) -> None:
        expected = {"title": "The Element in Half the Worlds", "subject": "Frankl's Union-Closed Sets Conjecture", "pages": "48", "date": "2026-07-27"}
        labels = {"title": "Title", "subject": "Subject", "pages": "Pages", "date": "Source-lock date"}
        for key, value in expected.items():
            self.assertRegex(self.source, rf"(?m)^% {re.escape(labels[key])}: {re.escape(value)}$")
