...
class AdministrativeReview0121LateRecoveryTests(unittest.TestCase):
    ...
    def expected_pair(self):
        ...
        pull = {"number": 523, "state": "open", "head": {"ref": c["candidate_branch"], "sha": c["stale_finalized_head"]}}
        manifest = {
    ...
    def incomplete_ledger(self):
        return {"procedures": {"administrative_review": {"completed_through_utc": "2026-08-13T01:21:00Z", "receipts": []}}}
    ...
    def test_schema_and_exact_boundary(self):
        ...
        self.assertEqual(control["occurrence"]["record_id"], "MP-ADMIN-ADMINISTRATIVE-REVIEW-2026-08-16-001")
    ...
    def test_bounded_window_is_next_admin_locus(self):
        control = self.load_control()
        self.assertEqual(recovery.bounded_recovery_minutes(self.runtime(), control), 4320)
        self.assertEqual(control["occurrence"]["bounded_recovery_expires_at_utc"], "2026-08-19T01:21:00Z")

    def test_existing_chain_wins(self):
        ordinary = ({"number": 999}, {"occurrence_key": "other"})
        result = recovery.eligible_candidates(object(), "grandchallenge/MATH-PROGRAMME", self.runtime(), datetime(2026, 8, 17, 12, 0, tzinfo=UTC), base=lambda *args: [ordinary])
    ...
    def test_exact_candidate_only_inside_bounded_window(self):
        ...
        def base(candidate, repo, runtime, now):
            if runtime["scope"]["recovery_window_minutes_after_due"] > 180:
                return [({"number": 520}, {"occurrence_key": "structural_sweep:2026-08-15T18:09:00Z"}), pair]
            return []
        kwargs = dict(candidate=object(), repo="grandchallenge/MATH-PROGRAMME", runtime=self.runtime(), base=base, completion_loader=lambda *_: self.incomplete_ledger(), ancestry_checker=lambda *_: True)
        self.assertEqual(recovery.eligible_candidates(now=datetime(2026, 8, 16, 4, 20, tzinfo=UTC), **kwargs), [])
        self.assertEqual(recovery.eligible_candidates(now=datetime(2026, 8, 17, 12, 0, tzinfo=UTC), **kwargs), [pair])
        self.assertEqual(recovery.eligible_candidates(now=datetime(2026, 8, 19, 1, 21, tzinfo=UTC), **kwargs), [])

    def test_completion_suppresses_recovery(self):
        pair = self.expected_pair()
        complete = {"procedures": {"administrative_review": {"completed_through_utc": "2026-08-16T01:21:00Z", "receipts": [{"scheduled_due_at": "2026-08-16T01:21:00Z"}]}}}
        result = recovery.eligible_candidates(object(), "grandchallenge/MATH-PROGRAMME", self.runtime(), datetime(2026, 8, 17, 12, 0, tzinfo=UTC), base=lambda candidate, repo, runtime, now: [pair] if runtime["scope"]["recovery_window_minutes_after_due"] > 180 else [], completion_loader=lambda *_: complete, ancestry_checker=lambda *_: True)
    ...
    def test_ancestry_and_identity_drift_fail_closed(self):
        ...
        with self.assertRaises(AutonomyError):
            recovery.eligible_candidates(object(), "grandchallenge/MATH-PROGRAMME", self.runtime(), datetime(2026, 8, 17, 12, 0, tzinfo=UTC), base=base, completion_loader=lambda *_: self.incomplete_ledger(), ancestry_checker=lambda *_: False)
        pull, manifest = pair
        drifted = copy.deepcopy(manifest)
        drifted["pull_request_number"] = 999
        with self.assertRaises(AutonomyError):
            recovery.eligible_candidates(object(), "grandchallenge/MATH-PROGRAMME", self.runtime(), datetime(2026, 8, 17, 12, 0, tzinfo=UTC), base=lambda candidate, repo, runtime, now: [(pull, drifted)] if runtime["scope"]["recovery_window_minutes_after_due"] > 180 else [], completion_loader=lambda *_: self.incomplete_ledger(), ancestry_checker=lambda *_: True)
    ...
    def test_governed_sync_head_may_advance(self):
        ...
        result = recovery.eligible_candidates(object(), "grandchallenge/MATH-PROGRAMME", self.runtime(), datetime(2026, 8, 17, 12, 0, tzinfo=UTC), base=lambda candidate, repo, runtime, now: [(advanced, manifest)] if runtime["scope"]["recovery_window_minutes_after_due"] > 180 else [], completion_loader=lambda *_: self.incomplete_ledger(), ancestry_checker=lambda *_: True)
    ...
    def test_runtime_entrypoint_chains_exact_wrapper(self):
        source = RUNTIME_ENTRY.read_text(encoding="utf-8")
        self.assertIn("administrative_autonomy_runtime_administrative_review_0121_recovery", source)
