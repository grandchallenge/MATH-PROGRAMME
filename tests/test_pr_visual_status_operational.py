...
class FakeClient:
    ...
    def get(self, path: str):
        normalized = path.replace("&per_page=100&page=1", "").replace("?per_page=100&page=1", "")
...
class PRVisualStatusOperationalTests(unittest.TestCase):
    ...
    def test_significance_selects_workflow_and_not_ordinary_docs(self) -> None:
    ...
    def test_governed_manual_override_selects_ordinary_pr(self) -> None:
    ...
    def test_readable_ruleset_without_pull_request_rule_has_no_github_review_requirement(self) -> None:
        contexts, review = operational.ruleset_requirements(
            [{"rules": [{"type": "required_status_checks", "parameters": {
                "required_status_checks": [{"context": "Programme policy checks"}]
            }}]}]
        )
        self.assertEqual({"Programme policy checks"}, contexts)
        self.assertIs(review, False)

    def test_review_authority_ignores_old_head_approval(self) -> None:
        client = FakeClient({
            "/repos/grandchallenge/MATH-PROGRAMME/pulls/99/reviews?": [{
                "id": 1, "state": "APPROVED", "commit_id": OLD_HEAD,
                "submitted_at": "2026-08-12T00:00:00Z", "user": {"login": "reviewer"},
            }]
        })
    ...
    def test_review_authority_accepts_current_non_author_review(self) -> None:
        client = FakeClient({
            "/repos/grandchallenge/MATH-PROGRAMME/pulls/99/reviews?": [{
                ...
                "submitted_at": "2026-08-12T00:01:00Z", "user": {"login": "reviewer"},
            }]
        })
        pr = {"number": 99, "head": {"sha": HEAD}, "user": {"login": "author"}}
    ...
    def test_human_steward_requires_configured_actor_and_exact_head(self) -> None:
        client = FakeClient({
            "/repos/grandchallenge/MATH-PROGRAMME/issues/99/comments?": [
                {"id": 10, "body": f"HUMAN_STEWARD_AUTHORIZED_EXACT_HEAD_PROTECTED_MERGE\n{OLD_HEAD}",
                 "created_at": "2026-08-12T00:00:00Z", "user": {"login": "fyremael"}},
                {"id": 11, "body": f"HUMAN_STEWARD_AUTHORIZED_EXACT_HEAD_PROTECTED_MERGE\n{HEAD}",
                 "created_at": "2026-08-12T00:01:00Z", "user": {"login": "not-steward"}},
            ]
        })
        pr = {"number": 99, "head": {"sha": HEAD}}
        authority, observed = operational.steward_authority(
            client, "grandchallenge/MATH-PROGRAMME", pr, {"human_steward_logins": ["fyremael"]}
        )
        self.assertEqual("UNKNOWN", authority["state"])
        self.assertEqual(2, len(observed))

    def test_human_steward_accepts_exact_configured_actor(self) -> None:
        client = FakeClient({
            "/repos/grandchallenge/MATH-PROGRAMME/issues/99/comments?": [{
                "id": 12, "body": f"HUMAN_STEWARD_AUTHORIZED_EXACT_HEAD_PROTECTED_MERGE\n{HEAD}",
                "created_at": "2026-08-12T00:02:00Z", "user": {"login": "fyremael"},
            }]
        })
        pr = {"number": 99, "head": {"sha": HEAD}}
    ...
    def test_cross_repository_collection_is_rejected_before_source_reads(self) -> None:
        with self.assertRaisesRegex(operational.OperationalError, "outside Phase 1 authority"):
            operational.collect_report(
                FakeClient({}), "grandchallenge/MATHFORGE", 99,
                "2026-08-12T00:00:00Z",
                {"repository": "grandchallenge/MATH-PROGRAMME", "manual_override_path_template": "x/{pr_number}.json"},
            )

    def test_workflow_is_nonblocking_and_checks_out_trusted_base(self) -> None:
        text = (ROOT / ".github" / "workflows" / "pr-visual-status-advisory.yml").read_text(encoding="utf-8")
