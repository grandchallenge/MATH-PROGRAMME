...
class AdministrativeAutonomyTests(unittest.TestCase):
    def setUp(self) -> None:
        self.record = json.loads(
            (ROOT / "governance" / "administrative_autonomy_transition.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_candidate_and_referee_must_be_distinct(self) -> None:
        ...
        mutated["agents"]["referee"]["expected_login"] = mutated["agents"]["candidate"][
            "expected_login"
        ]
        self.assertIn(
    ...
    def test_human_steward_impersonation_is_rejected(self) -> None:
        mutated = copy.deepcopy(self.record)
        mutated["delegated_authority"]["automated_human_steward_disposition"] = True
        self.assertIn(
    ...
    def test_direct_push_bypass_is_rejected(self) -> None:
        mutated = copy.deepcopy(self.record)
        mutated["delegated_authority"]["branch_protection_bypass"]["direct_push"] = True
        self.assertIn(
    ...
    def test_silent_cadence_rewrite_is_rejected(self) -> None:
        mutated = copy.deepcopy(self.record)
        mutated["delegated_authority"]["cadence_anchor_reset"]["silent_rewrite"] = True
        self.assertIn(
