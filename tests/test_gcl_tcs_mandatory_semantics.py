...
class GclTcsMandatorySemanticTests(unittest.TestCase):
    ...
    def required_targets(self) -> dict[str, tuple[dict, dict]]:
        ...
        targets = {
            ...
            "declaration.dependency": ({
                "artifact_id": "DEP-001", "version_or_revision": "abc123",
                "authority_status": "authoritative", "relation": "source",
            }, defs["dependency"]),
            ...
            "declaration.conformance_dimensions": (
                copy.deepcopy(self.decl_template["conformance_dimensions"]), dimension_schema
            ),
            "declaration.licence_and_access": (
                copy.deepcopy(self.decl_template["licence_and_access"]), props["licence_and_access"]
            ),
            "declaration.generated_content": (
                copy.deepcopy(self.decl_template["generated_content"]), props["generated_content"]
            ),
        }
        for name, template_key, schema_key in (
    ...
    def test_coverage_contract_is_exact_and_governed(self) -> None:
        self.assertEqual(coverage_contract_errors(
    ...
    def test_every_required_field_rejects_omission(self) -> None:
        ...
        for target_name, fields in self.coverage["required_field_sets"].items():
    ...
    def test_matrix_bound_field_semantics_reject_invalid_values(self) -> None:
        ...
        for name, mutate in (
            ("schema_version_const", lambda x: x.__setitem__("schema_version", "latest")),
            ("standard_id_const", lambda x: x["standard"].__setitem__("id", "GCL-TCS-99")),
            ("standard_version_const", lambda x: x["standard"].__setitem__("version", "latest")),
            ("profile_id_enum", lambda x: x["primary_profile"].__setitem__("id", "GCL-TCS-P99")),
            ("profile_version_const", lambda x: x["primary_profile"].__setitem__("version", "latest")),
            ("authority_status_enum", lambda x: x.__setitem__("authority_status", "displayed")),
            ("promotion_status_enum", lambda x: x.__setitem__("promotion_status", "approved")),
            ("impact_class_enum", lambda x: x.__setitem__("impact_class", "IC-4")),
            ("assessment_state_enum", lambda x: x["conformance_dimensions"].__setitem__("V", "COMPLETE")),
            ("date_format", lambda x: x.__setitem__("date", "2026-99-99")),
            ("artifact_id_pattern", lambda x: x.__setitem__("artifact_id", "bad id with spaces")),
            ("audience_min_items", lambda x: x.__setitem__("audience", [])),
            ("secondary_profiles_unique", lambda x: x.__setitem__(
                "secondary_profiles", [copy.deepcopy(x["primary_profile"]), copy.deepcopy(x["primary_profile"])]
            )),
            ("additional_properties_closed", lambda x: x.__setitem__("display_authority", "promoted")),
            ("not_applicable_requires_reason", lambda x: x.__setitem__(
                "terminology_registry", {"status": "not_applicable"}
            )),
            ("explicit_empty_requires_reason", lambda x: x.__setitem__(
                "claim_register", {"status": "explicit_empty"}
            )),
        ):
            declaration_case(name, mutate)

        defs = self.decl_schema["$defs"]
        cases.extend([
            ("dependency_authority_enum", {
                "artifact_id": "DEP-1", "version_or_revision": "abc",
                "authority_status": "working", "relation": "source",
            }, defs["dependency"]),
            ("review_reference_gate_pattern", {
                "review_id": "R", "gate_id": "G10", "location": "r",
                "decision": "PASS", "reviewed_revision": "abc",
            }, defs["reviewReference"]),
        ...
        record_defs = self.record_schema["$defs"]
        for name, template_key, schema_key, mutate in (
            ...
            ("evidence_datetime_format", "evidence_record", "evidenceRecord", lambda x: x.__setitem__("created_at", "not-a-date-time")),
    ...
    def test_claim_requires_exactly_one_statement_or_pointer(self) -> None:
        ...
        pointer_only = copy.deepcopy(missing)
    ...
    def test_assured_requires_linked_exact_revision_review(self) -> None:
        ...
        declaration["review_register"] = [{
            "review_id": "REV-ASSURED-001", "gate_id": "G5",
            "location": "reviews/REV-ASSURED-001.yaml", "decision": "PASS",
    ...
    def test_gate_review_semantics_fail_closed(self) -> None:
        ...
        self.assertIn("gate_review: NOT_APPLICABLE_requires_reviewer_approval", gate_review_semantic_errors(gate, review))
    ...
    def test_dedicated_exception_semantics_are_reused_and_governed(self) -> None:
        ...
        self.assertTrue(any(
            item["test"] == EXCEPTION_TEST for item in self.coverage["reused_dedicated_controls"]
        ))
