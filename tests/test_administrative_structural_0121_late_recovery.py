...
SCHEMA = (
    ROOT
    / "schemas"
    / "administrative_structural_0121_late_recovery_control.schema.json"
)
...
class AdministrativeStructural0121LateRecoveryTests(unittest.TestCase):
    ...
    def test_schema_and_exact_hole_boundary(self):
        control = self.load_control()
    ...
    def test_exact_hole_is_absent_despite_later_frontier(self):
        self.assertTrue(
            recovery.exact_receipt_absent(
                self.later_frontier_ledger(), self.load_control()
            )
        )
        ledger = self.later_frontier_ledger()
        ledger["procedures"]["structural_sweep"]["receipts"].insert(
            1, self.target_receipt()
        )
        ledger["procedures"]["structural_sweep"]["receipt_count"] = 3
        self.assertFalse(recovery.exact_receipt_absent(ledger, self.load_control()))
    ...
    def test_bounded_window_is_exact_and_does_not_change_global_window(self):
        runtime = self.runtime()
        self.assertEqual(
            recovery.bounded_recovery_minutes(runtime, self.load_control()), 3024
        )
        self.assertEqual(runtime["scope"]["recovery_window_minutes_after_due"], 180)

    def test_ordinary_eligibility_has_precedence(self):
    ...
    def test_widened_replay_filters_exact_515_and_allows_post_sync_head(self):
        ...
        def base(candidate, repo, runtime, now):
            ...
            calls.append(window)
            return [] if window == 180 else [expected, (other_pull, other_manifest)]
        ...
        self.assertNotEqual(
            expected[0]["head"]["sha"],
            self.load_control()["occurrence"]["stale_finalized_head"],
        )

    def test_recovery_expires_at_bounded_horizon(self):
        called = []

        def base(candidate, repo, runtime, now):
            called.append(runtime["scope"]["recovery_window_minutes_after_due"])
            return []
        ...
        self.assertEqual(called, [180])

    def test_source_ancestry_is_required(self):
    ...
    def test_backfill_inserts_sorted_receipt_without_lowering_frontier(self):
        ...
        updated = recovery.advance_completion_state(
            current, receipt, "4" * 40
        )
        procedure = updated["procedures"]["structural_sweep"]
        self.assertEqual(procedure["receipt_count"], 3)
        self.assertEqual(
            procedure["completed_through_utc"], "2026-08-16T10:57:00Z"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            recovery.advance_completion_state(updated, receipt, "5" * 40), updated
        )

    def test_backfill_rejects_conflicting_same_due(self):
        current = self.later_frontier_ledger()
    ...
    def test_non_target_receipt_delegates(self):
        sentinel = {"delegated": True}
        result = recovery.advance_completion_state(
            self.later_frontier_ledger(),
            self.receipt(
                "2026-08-17T03:45:00Z",
                "governance/administrative_structural_sweeps/later.json",
                999,
            ),
            "4" * 40,
    ...
    def test_runtime_wiring_patches_selection_receipt_and_mirror_paths(self):
        ...
        self.assertIsNot(receipt_stage.advance_completion_state, recovery.advance_completion_state)
        self.assertIsNot(receipt_resume.advance_completion_state, recovery.advance_completion_state)
