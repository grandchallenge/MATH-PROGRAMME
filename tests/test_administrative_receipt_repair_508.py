...
SCHEMA_PATH = ROOT / "schemas" / "administrative_receipt_repair_508.schema.json"
RECORD_PATH = (
    ROOT
    / "governance"
...
class AdministrativeReceiptRepair508Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.receipt = next(
            ...
            if item.get("repair_id") == "MP-ADMIN-RECEIPT-REPAIR-508-001"
        )
    ...
    def test_bootstrap_derivation_recovers_only_the_exact_receipt(self) -> None:
        with patch.object(receipts, "git_blob_sha", return_value=self.receipt["record_git_blob"]), patch.object(
            ...
            normalized = receipts.normalize_repaired_bootstrap_receipt_508(
                ROOT,
                self.receipt,
                "a" * 40,
                self.git_fixture,
            )
        self.assertEqual(508, normalized["pull_request"])
        self.assertEqual(
            "55af16e632f87b1b6f707d633732938b1df215dc",
    ...
    def test_evidence_mutation_fails_closed(self) -> None:
        ...
        with patch.object(receipts, "git_blob_sha", return_value=self.receipt["record_git_blob"]), patch.object(
            ...
            with self.assertRaises(receipts.aa.AutomationError):
                receipts.normalize_repaired_bootstrap_receipt_508(
                    ROOT,
                    repaired,
                    "a" * 40,
                    self.git_fixture,
