...
lemma rankOneMultiplication_identity_evalOne (X : CompHaus.{u}) :
  ...
  ext x
  change
    (((show LocallyConstant X R from a) x) * (1 : R)).down =
      ((show LocallyConstant X R from a) x).down
  exact congrArg ULift.down (mul_one ((show LocallyConstant X R from a) x))
  ...
  change
    (rankOneMultiplicationApp X ≫
      CategoryTheory.Enriched.FunctorCategory.enrichedHomπ
        ...
        (Under.forget (op X) ⋙ coefficientPresheaf)
        (Under.forget (op X) ⋙ coefficientPresheaf)
        (Under.mk (𝟙 (op X)))) ≫ rankOneEndomorphismEvalOne X =
      𝟙 (coefficientAt X)
  rw [rankOneMultiplicationApp_projection]
