...
lemma uliftInt_smul_eq_zsmul
  ...
  letI : Module R M := mM
  obtain ⟨z⟩ := r
  have hcast : (ULift.up z : R) = (z : R) := by
    simpa using (ULift.up_intCast (R := ℤ) z)
  rw [hcast]
          ...
          projectionLinearMap A T (Under.mk (𝟙 (op T))) (φ + ψ)
            ...
            projectionLinearMap A T (Under.mk (𝟙 (op T))) φ
              ...
              projectionLinearMap A T (Under.mk (𝟙 (op T))) ψ
                (LocallyConstant.const T a)
        rw [projectionLinearMap_add A T (Under.mk (𝟙 (op T))) φ ψ]
        rfl }
  ...
  let f : AddHom (familyModule A T)
    ...
    { toFun := fun L => reconstructedLinearMap A T L k
      map_add' := by
        ...
        apply LocallyConstant.ext
        intro y
        simp only [reconstructedLinearMap, ModuleCat.homAddEquiv,
          ModuleCat.homEquiv]
