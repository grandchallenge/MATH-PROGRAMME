...
The following example is informative. The machine-readable templates in the version-1 release package are normative for version 1.0.0 when that exact release is admitted through G9.
...
generated_content:
  ...
  review: "Technical review required before promotion"
```

# 20. Acceptance criteria for GCL-TCS-00 version 1.0

Version-1 authority can become effective only when:
...
10. The Referee approves promotion of the exact version-1 release candidate with an explicit statement of remaining limitations.
11. G9 atomically admits the exact Referee-approved release package and its required records before authority or publication is claimed.

Criteria 1–9 may reuse evidence from the 0.1.0 bounded-candidate programme only when the reviewing gate verifies that the evidence remains applicable to the exact version-1 revision. No historical readiness record substitutes for an exact-revision gate decision.
