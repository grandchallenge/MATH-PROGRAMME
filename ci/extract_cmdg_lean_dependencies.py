...
def load_config(path: Path) -> dict[str, Any]:
    ...
    required = {
        ...
        "module",
        "roots",
        "expected_toolchain_git_blob_sha1",
        "expected_lake_manifest_git_blob_sha1",
        "expected_axioms",
        "claim_boundary",
    }
...
def validate_pins(config: dict[str, Any], project_dir: Path) -> dict[str, str]:
    ...
    if actual_toolchain != config["expected_toolchain_git_blob_sha1"]:
        reject(
            "STALE_TOOLCHAIN_PIN",
            f"lean-toolchain blob drift: {actual_toolchain} != {config['expected_toolchain_git_blob_sha1']}",
        )
    if actual_manifest != config["expected_lake_manifest_git_blob_sha1"]:
        reject(
            "STALE_LAKE_MANIFEST_PIN",
            f"lake-manifest blob drift: {actual_manifest} != {config['expected_lake_manifest_git_blob_sha1']}",
        )
    return {
...
private def sortedNames (s : NameSet) : Array Name :=
...
private def mergeNames (a b : Array Name) : Array Name := Id.run do
  ...
  return sortedNames s

private def parts (ci : ConstantInfo) : Array Name × Array Name :=
  let typeDeps (e : Expr) := sortedNames e.getUsedConstants
  match ci with
  ...
  | .inductInfo v => (typeDeps v.type, v.ctors)
...
private def moduleOf? (env : Environment) (n : Name) : Option Name :=
  ...
  | some idx => env.header.moduleNames.get? idx
...
private def directImports (env : Environment) (n : Name) : Array Name :=
  match env.getModuleIdxFor? n with
  | some idx =>
      match env.header.moduleData.get? idx with
...
elab_rules : command
  | `(#cmdg_extract $id:ident) => do
      ...
      let localClosure := (seen.erase root |> sortedNames)
      ...
      for n in sortedNames frontier do
        let kind := match env.find? n with | some c => kindString c | none => "unknown"
        let modName := (moduleOf? env n).getD `_unknown
    ...
    for line in lines:
        ...
        if tag in {"ROOT", "KIND", "MODULE", "SEMANTIC_AUTHORITY", "GRAPH_CERTIFIED"}:
            ...
            if len(parts) != 5:
                reject("PROBE_OUTPUT_MALFORMED", line)
            result["edges"].append(
                {"source": parts[2], "target": parts[3], "target_module": parts[4]}
            )
        else:
...
def run_probe(project_dir: Path, module: str, root: str) -> dict[str, Any]:
    source = lean_probe_source(module, root)
    probe_path: Path | None = None
    try:
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".lean", prefix="CMDGExtract_", dir=project_dir, encoding="utf-8", delete=False
        ) as handle:
            handle.write(source)
...
def build_root_report(root: str, probe: dict[str, Any]) -> dict[str, Any]:
    ...
    axioms = probe["axioms"]
    classicality = [ax for ax in axioms if ax == "Classical.choice"]
    common = {name: name in axioms for name in COMMON_AXIOMS}
    direct_edges = [
        {
            **edge,
            "layer": "G_proof",
    ...
    return {
        ...
        "observed_direct_proof_edges": direct_edges,
        ...
        "axiom_footprint": {
            "axioms": axioms,
            "classicality_dependencies": classicality,
            "common_axioms": common,
...
def validate_expected_axioms(config: dict[str, Any], roots: list[dict[str, Any]]) -> None:
    ...
    if expected is None:
        reject("EXPECTED_AXIOM_FOOTPRINT_UNSET", "expected_axioms must be retained after bootstrap observation")
        ...
        "implementation_imports": sorted(
            {
                imp
                for root in config["roots"]
                for imp in run_probe(project_dir, config["module"], root)["imports"]
            }
        ),
        "authority_contract": {
