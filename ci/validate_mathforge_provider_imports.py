...
def mathforge_provider_import_errors(
    ...
    ids = [entry.get("campaign_id") for entry in campaigns if isinstance(entry, dict)]
    duplicates = sorted({campaign_id for campaign_id in ids if ids.count(campaign_id) > 1})
    ...
    for missing in sorted(expected_ids - actual_ids):
        errors.append(f"MATHFORGE imports: active campaign is uncovered: {missing}")
    for unknown in sorted(actual_ids - expected_ids):
        errors.append(f"MATHFORGE imports: unregistered active-campaign authority: {unknown}")
    ...
    for campaign_id, expected in EXPECTED_IMPORTS.items():
        ...
        if entry.get("manifest_git_blob_sha1") != blob_sha:
            errors.append(
                ...
                f"expected {blob_sha}, found {entry.get('manifest_git_blob_sha1')!r}"
            )
...
def provider_gate_errors(
    ...
    if entry.get("disposition") == "import":
        required = ("coverage_mode", "manifest_path", "manifest_git_blob_sha1")
        missing = [field for field in required if not entry.get(field)]
        return [f"{campaign_id} {stage}: incomplete MATHFORGE import fields: {', '.join(missing)}"] if missing else []
    waiver = entry.get("waiver")
    if entry.get("disposition") == "waiver" and isinstance(waiver, dict):
        ...
        return [f"{campaign_id} {stage}: incomplete MATHFORGE waiver fields: {', '.join(missing)}"] if missing else []
...
def main() -> int:
    ...
    if errors:
        ...
        print(f"MATHFORGE provider import validation failed with {len(errors)} error(s)", file=sys.stderr)
        return 1
    print(
        "MATHFORGE provider imports are pinned: 8 campaigns, exact commit, "
        "manifest paths, content identities, and promotion coverage"
    )
    return 0
