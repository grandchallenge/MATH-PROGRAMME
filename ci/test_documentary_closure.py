...
def main() -> int:
    ...
    assert any(
        ...
        for error in agent_review_terminal_errors(incomplete_terminal, "DOCS-DOCUMENTARY-001")
    )

    missing_terminal_evidence = copy.deepcopy(published)
    missing_terminal_evidence["amanuensis_control"]["review_provenance"]["evidence_refs"] = []
    assert any(
        "at least one evidence reference" in error
        for error in agent_review_terminal_errors(missing_terminal_evidence, "DOCS-DOCUMENTARY-001")
    )

    unresolved_terminal_conflict = copy.deepcopy(published)
    unresolved_terminal_conflict["amanuensis_control"]["cross_document_consistency"]["conflicts"] = ["synthetic conflict"]
    assert any(
        "must not be hidden" in error
        for error in agent_review_terminal_errors(unresolved_terminal_conflict, "DOCS-DOCUMENTARY-001")
    )

    missing_integrated_artifact = copy.deepcopy(published)
    missing_integrated_artifact["amanuensis_control"]["final_editorial_integration"]["integrated_artifact_ref"] = "docs/DOES-NOT-EXIST.md"
    assert any(
        "authoritative integrated artifact does not resolve" in error
        for error in agent_review_terminal_errors(missing_integrated_artifact, "DOCS-DOCUMENTARY-001")
    )
    ...
    assert not evidence_package_coverage_errors(discovered_packages, registered_contracts, registered_legacy)
    ...
    assert any(
        "MP-FUTURE-UNCONTRACTED-001" in error and "lacks a registered closure_contract.json" in error
        for error in evidence_package_coverage_errors(synthetic_uncontracted, registered_contracts, registered_legacy)
    )
    ...
    assert any(
        ...
        for error in evidence_package_coverage_errors(discovered_packages | synthetic_legacy_expansion, registered_contracts, synthetic_legacy_expansion)
    )

    assert any(
        "fixed legacy evidence baseline entry is missing" in error
        for error in evidence_package_coverage_errors(discovered_packages, registered_contracts, set())
    )
    ...
    assert any(
        ...
        for error in evidence_package_coverage_errors(discovered_packages, registered_contracts, legacy_overlap)
    )

    low_friction_path = ROOT / registry["contracts"][0]
    low_friction = load_json(low_friction_path)
    assert low_friction["binding"]["status"] == CANONICAL_CLOSURE_STATUS
    assert low_friction["admission"]["phase"] == "protected"
    assert not closure_contract_errors(low_friction, registry["contracts"][0])
    ...
    bad_consistency_ref["cross_document_consistency"]["checked_against"].append("governance/DOES-NOT-EXIST.json")
    assert any(
        "consistency reference does not resolve" in error
        for error in closure_contract_errors(bad_consistency_ref, "synthetic")
    )

    bad_readback = copy.deepcopy(low_friction)
    bad_readback["admission"]["protected_main_readback"] = "0" * 40
    assert any(
        "protected_main_readback must equal protected_merge" in error
        for error in closure_contract_errors(bad_readback, "synthetic")
    )

    missing_review_binding = copy.deepcopy(low_friction)
    review_id = missing_review_binding["admission"]["independent_review_id"]
    missing_review_binding["terminal_evidence_refs"] = [
        ref for ref in missing_review_binding["terminal_evidence_refs"] if review_id not in ref
    ]
    assert any(
        "canonical terminal evidence does not bind admission token" in error and review_id in error
        for error in closure_contract_errors(missing_review_binding, "synthetic")
    )

    candidate = copy.deepcopy(low_friction)
    candidate["binding"]["status"] = CANDIDATE_CLOSURE_STATUS
    candidate["admission"] = {
        "phase": "candidate",
        "required_next_step": "protected_pr_admission_and_readback",
    }
    candidate["terminal_evidence_refs"] = []
    candidate["unresolved_documentary_obligations"] = ["protected_pr_admission_and_readback"]
    assert not closure_contract_errors(candidate, "candidate")

    premature_canonical = copy.deepcopy(candidate)
    premature_canonical["binding"]["status"] = CANONICAL_CLOSURE_STATUS
    assert closure_contract_errors(premature_canonical, "premature-canonical")

    candidate_with_terminal_claim = copy.deepcopy(candidate)
    candidate_with_terminal_claim["terminal_evidence_refs"] = ["future protected merge"]
    assert closure_contract_errors(candidate_with_terminal_claim, "candidate-terminal-claim")
