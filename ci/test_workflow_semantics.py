#!/usr/bin/env python3
"""Adversarial tests for semantic workflow, routing, replay, and publication contracts."""
...
def main() -> int:
    ...
    for step in missing_classifier["ci.yml"]["jobs"]["impact"]["steps"]:
        if "policy_impact.py classify" in str(step.get("run", "")):
            ...
            break
    assert any(
        "impact classifier" in error
        for error in workflow_semantic_errors(workflows=missing_classifier)
    )
    ...
    for step in shallow_classifier["ci.yml"]["jobs"]["impact"]["steps"]:
        if str(step.get("uses", "")).startswith("actions/checkout@"):
            ...
            break
    assert any(
        "full-history checkout" in error
    ...
    assert any(
        "aggregate impact and policy-shard" in error
        for error in workflow_semantic_errors(workflows=missing_aggregate)
    )

    unsafe_aggregate = copy.deepcopy(workflows)
    unsafe_aggregate["ci.yml"]["jobs"]["validate-json"]["if"] = "success()"
    assert any(
        "run under always()" in error
        for error in workflow_semantic_errors(workflows=unsafe_aggregate)
    )
    ...
    for step in missing_formal_gate["ci.yml"]["jobs"]["log-gcd-lean"]["steps"]:
        if "formal_replay_gate.py decide" in str(step.get("run", "")):
            ...
            break
    assert any(
        "formal replay impact-gate" in error
        for error in workflow_semantic_errors(workflows=missing_formal_gate)
    )
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        assert any("repository-regression shard is missing" in error for error in errors), errors
