...
def require_error(texts: dict[str, str], evidence: dict, needle: str) -> None:
    errors = workflow_coverage_errors(texts=texts, evidence=evidence)
...
def main() -> int:
    ...
    require_error(referee_removed, evidence, "Referee token")
    ...
    require_error(admin_scope_removed, evidence, "administration-write")
    ...
    require_error(runtime_removed, evidence, "runtime marker")
    ...
    require_error(permission_drift, evidence, "delegated Referee permissions drift")

    direct_push = dict(texts)
    direct_push["administrative-maintenance-candidate.yml"] += "\n# git push origin main\n"
    require_error(direct_push, evidence, "forbidden runtime capability")
    ...
    require_error(sync_v4_removed, evidence, "bounded synchronization marker")

    arbitrary_manual_sha = dict(texts)
    arbitrary_manual_sha["administrative-maintenance-synchronization.yml"] += "\n# inputs.head_sha\n"
    require_error(arbitrary_manual_sha, evidence, "manual arbitrary-SHA")
    ...
    require_error(activation_permissions_drift, evidence, "activate job delegated permissions drift")
    ...
    require_error(activation_environment_removed, evidence, "activate job must bind")
