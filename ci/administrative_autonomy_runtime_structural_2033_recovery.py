...
CONTROL_PATH = ROOT / "governance" / "administrative_structural_2033_late_recovery_control.json"
UTC = timezone.utc
...
def bounded_recovery_minutes(runtime: dict[str, Any], control: dict[str, Any]) -> int:
    ...
    expires = parse_datetime(control["occurrence"]["bounded_recovery_expires_at_utc"])
    ...
    if ordinary != declared:
        raise AutonomyError("20:33 recovery control does not match protected ordinary recovery window")
    if bounded <= ordinary:
        raise AutonomyError("20:33 recovery continuation does not extend beyond ordinary recovery")
    return bounded
...
def eligible_candidates(
    ...
    """Admit only the missed 20:33 structural candidate after ordinary expiry.

    Ordinary protected runtime eligibility always wins.  This wrapper never
    mutates the protected runtime configuration; it uses a temporary in-memory
    horizon only for the exact governed #400/#401 occurrence, and only until
    the next anchored structural locus.
    """
    ...
    if now.tzinfo is None:
        raise AutonomyError("20:33 recovery evaluation time must include an offset")
    observed = now.astimezone(UTC)
    control = load_control()
    if control.get("control_id") != "MP-ADMIN-STRUCTURAL-2033-LATE-RECOVERY-001":
        raise AutonomyError("20:33 recovery control identity drift")
    if control.get("status") != "ACTIVE_ON_PROTECTED_MERGE":
        return []

    ordinary_expiry = parse_datetime(control["occurrence"]["ordinary_recovery_expires_at_utc"])
    bounded_expiry = parse_datetime(control["occurrence"]["bounded_recovery_expires_at_utc"])
    ...
    if not ancestry_checker(candidate, repo, source):
        raise AutonomyError("20:33 recovery source is not ancestral to protected main")
    ...
    for pull, manifest in replayed:
        ...
        if int(manifest.get("issue_number") or 0) != expected_issue:
            raise AutonomyError("20:33 recovery issue identity drift")
        if int(manifest.get("pull_request_number") or 0) != expected_pr:
            raise AutonomyError("20:33 recovery manifest PR identity drift")
        if int(pull.get("number") or 0) != expected_pr:
            raise AutonomyError("20:33 recovery pull-request identity drift")
        if str(manifest.get("branch") or "") != expected_branch:
            raise AutonomyError("20:33 recovery branch identity drift")
        if str(manifest.get("source_protected_head") or "") != source:
            raise AutonomyError("20:33 recovery source-head identity drift")
        matches.append((pull, manifest))

    if len(matches) > 1:
        raise AutonomyError("duplicate exact 20:33 recovery candidate")
    return matches
