...
def bounded_recovery_minutes(
    ...
    if ordinary != declared:
        raise AutonomyError(
            "08:33 recovery control does not match protected ordinary "
            "recovery window"
        )
    if bounded <= ordinary:
        raise AutonomyError(
            "08:33 recovery continuation does not extend beyond ordinary "
            "recovery"
        )
    return bounded
...
def eligible_candidates(
    ...
    """Admit only the missed 08:33 structural candidate.

    Ordinary protected eligibility and all earlier exact-occurrence recovery
    wrappers always win. This wrapper never changes the protected global
    ...
    if now.tzinfo is None:
        raise AutonomyError("08:33 recovery evaluation time must include an offset")
    observed = now.astimezone(UTC)
    control = load_control()
    if (
        control.get("control_id")
        != "MP-ADMIN-STRUCTURAL-0833-LATE-RECOVERY-001"
    ):
        raise AutonomyError("08:33 recovery control identity drift")
    ...
    if not ancestry_checker(candidate, repo, source):
        raise AutonomyError(
            "08:33 recovery source is not ancestral to protected main"
        )
    ...
    for pull, manifest in replayed:
        ...
        if int(manifest.get("issue_number") or 0) != expected_issue:
            raise AutonomyError("08:33 recovery issue identity drift")
        if int(manifest.get("pull_request_number") or 0) != expected_pr:
            raise AutonomyError("08:33 recovery manifest PR identity drift")
        if int(pull.get("number") or 0) != expected_pr:
            raise AutonomyError("08:33 recovery pull-request identity drift")
    ...
    if len(matches) > 1:
        raise AutonomyError("duplicate exact 08:33 recovery candidate")
    return matches
