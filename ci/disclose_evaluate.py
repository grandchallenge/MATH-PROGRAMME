...
def evaluate(fixture:Path)->dict[str,Any]:
    manifest=load(fixture/"manifest.json");reviews=load(fixture/manifest["evidence_files"]["reviews"]);head=manifest["candidate"]["head_sha"];out=[]
    ...
    out.append(finding("F-REVIEW-EXACT","PASS","EXACT_NON_AUTHOR_REVIEW",exact["review_id"],exact["reviewer"]) if exact["head_sha"]==head and not exact["author_account"] and exact["state"]=="APPROVED" and not exact["superseded"] else finding("F-REVIEW-EXACT","FAIL","REVIEW_NOT_CURRENT",exact["review_id"],exact["state"]))
