...
def main()->int:
    ...
    release_trust=impact.classify_paths(['ci/release_trust_admin.py','governance/release_trust_admin_contract.json','tests/test_intellect_profile_admin.py']);assert release_trust['policy_shards']==['core','administrative','contracts'];assert not release_trust['unknown_paths'];assert not any(release_trust['formal_dirty'].values())
    runner_control=impact.classify_paths(['ci/run_unittest_modules.py','ci/run_policy_shard.py']);assert runner_control['policy_shards']==['core','contracts'];assert not runner_control['unknown_paths'];assert not any(runner_control['formal_dirty'].values())
    classifier_control=impact.classify_paths(['ci/policy_impact.py','ci/test_policy_impact.py']);assert classifier_control['policy_shards']==['core','contracts'];assert not classifier_control['unknown_paths'];assert all(classifier_control['formal_dirty'].values())
    ...
    merge_group=impact.classify_paths([],event_name='merge_group');assert merge_group['event_mode']=='merge_group_full';assert merge_group['policy_shards']==list(impact.ALL_SHARDS);assert all(merge_group['formal_dirty'].values());assert not merge_group['unknown_paths']
