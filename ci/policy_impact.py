...
ALL_SHARDS=('core','fixtures','cmdg','administrative','campaigns','contracts','docs')
ALL_LANES=('log-gcd','pc-wp04','union-closed-mathcert')
...
def normalize_paths(paths:Iterable[str])->list[str]:
    ...
    for raw in paths:
        ...
        if not p or p.startswith('../') or '/../' in p: raise ImpactError(f'unsafe changed path: {raw!r}')
        out.append(p)
...
def formal_lane_impacts(paths:list[str],formal:dict,force_full:bool)->dict[str,bool]:
    ...
    global_inputs={str(x) for x in formal.get('global',{}).get('inputs',[])}
    if any(p in FULL_FANOUT_PATHS or p in global_inputs for p in paths): return {lane:True for lane in ALL_LANES}
    result={}
...
def classify_paths(paths:Iterable[str],*,event_name:str='pull_request',schedule:str|None=None)->dict:
    changed=normalize_paths(paths); control=load_json(CONTROL_PATH); formal=load_json(FORMAL_PATH)
    formal_cron=str(control['formal_replay']['sentinel']['cron']); full_cron=str(control['policy_dag']['full_policy_sentinel_cron'])
    clean={lane:False for lane in ALL_LANES}
...
def main()->int:
    ...
    except (ImpactError,OSError,json.JSONDecodeError,KeyError,ValueError) as exc: print(f'policy impact classification error: {exc}',file=sys.stderr); return 2
