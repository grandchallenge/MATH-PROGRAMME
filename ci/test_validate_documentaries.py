...
def run_rejection_tests() -> None:
    ...
    assert any(
        "latex_sha256" in error and "does not match manifest" in error
        for error in manifest_semantic_errors(mismatched_hash)
    )
    ...
    unfocusable_skip = page_text.replace(
        ' id="monograph-start" tabindex="-1"', ' id="monograph-start"', 1
    )
    assert any(
        "skip target must be programmatically focusable" in error
        for error in web_edition_errors(edition, unfocusable_skip)
    )

    unpinned_math = page_text.replace('crossorigin="anonymous"', '', 1)
    assert any(
        "MathJax policy attribute missing" in error
        for error in web_edition_errors(edition, unpinned_math)
    )
