...
def main() -> int:
    ...
    assert any("missing governed workflow ci.yml" in error for error in workflow_coverage_errors(texts=missing_policy, evidence=evidence))

    direct_pages_push = dict(texts)
    direct_pages_push["pages.yml"] = direct_pages_push["pages.yml"].replace(
        "  workflow_run:\n", "  push:\n    branches: [main]\n  workflow_run:\n", 1
    )
    assert any("triggered only by workflow_run" in error for error in workflow_coverage_errors(texts=direct_pages_push, evidence=evidence))
    ...
    assert any("missing publication gate" in error for error in workflow_coverage_errors(texts=bypass_success, evidence=evidence))
    ...
    assert any("validate_campaign_replays.py" in error for error in workflow_coverage_errors(texts=missing_replay, evidence=evidence))

    unpinned_evidence = copy.deepcopy(evidence)
    unpinned_evidence["commit"] = "main"
    assert any("does not match" in error for error in workflow_coverage_errors(texts=texts, evidence=unpinned_evidence))

    incomplete_evidence = copy.deepcopy(evidence)
    incomplete_evidence["paths"].remove("ci/replay_certificates.py")
    assert any("required formal and bounded replay paths are incomplete" in error for error in workflow_coverage_errors(texts=texts, evidence=incomplete_evidence))

    print("workflow coverage rejection tests passed")
    return 0
