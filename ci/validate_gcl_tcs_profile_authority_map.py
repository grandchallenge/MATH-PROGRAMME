...
def validate_profile_authority_map(mapping: Any, *, root: Path = ROOT) -> list[str]:
    ...
    _require("Roles identify review functions. They do not confer authority without a review record." in role_text,
             "role_source: non_conferral_clause_missing", errors)
    _require("| Steward | Standards selection, impact class, and lifecycle governance |" in role_text,
             "role_source: Steward_lifecycle_function_missing", errors)
...
def main() -> int:
    ...
    if errors:
        for error in errors:
            print(error)
        return 1
    print("GCL-TCS profile owner/reviewer authority map is a valid derivative projection of existing institutional roles")
    return 0
