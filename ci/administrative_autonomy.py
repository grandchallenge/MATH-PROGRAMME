...
from autonomy_github import (
    AutonomyError, Client, approve, auto_merge, branch, identity, install_bypass,
    pull, put_json, readback, required_contexts, restore_ruleset, set_workflow_approval,
    verify_scope, wait_checks, wait_merge, workflow_permissions,
)
...
def activation_record(t: dict[str, Any], head: str, candidate: Any, referee: Any, ruleset: int, prior: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "candidate_identity": candidate.record(), "referee_identity": referee.record(),
        "identity_separation": candidate.app_id != referee.app_id,
        "actions_approval": {
            "enabled": True,
        ...
        "auto_merge": {"repository_enabled": True, "method": "merge", "exact_head_required": True},
        "ruleset_bypass": {
        ...
        "rollback": {
            ...
            "actions_prior_state_captured": True, "failure_artifact_required": True,
        },
...
def activate(check_timeout: int, merge_timeout: int) -> int:
    ...
    if repo != t["repository"] or not sha(head) or os.getenv("GITHUB_REF") != "refs/heads/main":
        raise AutonomyError("activation must execute from the transition merge on protected main")
    candidate, referee, admin = (
        Client(os.getenv("CANDIDATE_TOKEN", "")),
        Client(os.getenv("REFEREE_TOKEN", "")),
        Client(os.getenv("ADMIN_TOKEN", "")),
    )
    ci, ri = identity(candidate), identity(referee)
    if ci.login != t["agents"]["candidate"]["expected_login"] or ri.login != t["agents"]["referee"]["expected_login"]:
        raise AutonomyError(f"agent identity drift: {ci.login}; {ri.login}")
    if ci.app_id == ri.app_id: raise AutonomyError("Candidate and Referee identities are not independent")
    ...
    before_ruleset, actions_changed, rules_changed, pr = None, False, False, None
    ...
    try:
        ...
        record = activation_record(t, head, ci, ri, ruleset_id, prior)
        ...
        readback(referee, repo, ACTIVATION_PATH, record)
        ...
        report |= {"state": "ACTIVATION_FAILED_CLOSED", "failed_at": now(), "error": str(exc), "rollback_errors": rollback, "authority_created": False}
        REPORT.write_text(json.dumps(report, indent=2) + "\n")
        raise
