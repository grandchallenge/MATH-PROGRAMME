...
Programme ruleset `17137629` now requires `formal-validation / formal-validation`. The three migration-only compatibility jobs have been removed after protected ruleset readback. Campaign-specific formal contexts are therefore no longer universal branch-protection requirements; substantive replay occurs only through the materially selected formal lanes and the separate promotion/sentinel surfaces.
