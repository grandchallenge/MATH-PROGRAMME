...
`ci/validate_documentary_closure.py` enforces these two routes in the contracts policy shard. It rejects unregistered closure contracts, stale registry entries, and any rebuild-evidence package that omits a registered closure contract unless that exact package is in the fixed legacy baseline.
