...
| Plane | A horizontal architectural band grouping functions with related authority and operational role. | A plane is not necessarily a software process, repository, pillar, or literal middleware hop. |
| MATH-CORE coordination plane | The shared MATH-CORE reasoning-state substrate holding checkpointed claims, obligations, conflicts, search constraints, scoped equivalences, witnesses, and certificates. | Not canonical mathematical truth, a fourth mathematical pillar, the Claim Ledger, the certification ladder, or a proof kernel. |
| INTELLECT control plane | The governed search/routing function that selects and allocates reasoning work over exact state. | Does not prove claims, self-authorize, or directly create canonical claim status. Persistent use remains subject to admitted controller and GH-OS routing requirements. |
| Stable pillar | A long-lived governed mathematical institution with a durable responsibility boundary; currently MATHFORGE, MATHSOLVE, and MATHCERT. | Stable does not mean immutable. Pillar creation, retirement, or material redefinition requires explicit governance. |
| Domain programme subgraph | A scoped, long-lived MATH-CORE claim/obligation graph with explicit programme/family identity and migration checkpoint. | Not a peer pillar; imported historical state is not retroactively represented as live MATH-CORE history. |
| Theory agent / reasoning service | A governed specialized capability that proposes derivations, conflicts, witnesses, equivalences, checks, or other bounded reasoning results against exact state. | Proposal capability does not confer blackboard mutation, certification, canonical-promotion, or Human Steward authority. |
| Transport fabric | Authority-neutral infrastructure carrying protocol state, messages, artifacts, or memory; GitHub/JSON/CI and future AETHER services are examples. | Transport identity or availability does not create semantic, mathematical, certification, or governance authority. |
| AETHER infrastructure plane | The orthogonal transport/memory fabric that may carry MATH-CORE/INTELLECT state and coordination traffic. | AETHER is not the reasoning semantics, proof boundary, canonical Claim Ledger, or policy authority. |
| Trusted acceptance boundary | The below-live-reasoning boundary containing distinct proof/replay checking, independent assurance, certification, canonical recording, and policy-disposition functions. | Not one monolithic checker; no component silently implies the others. |
| Canonical state | Governed claim state recorded through the canonical Claim Ledger and applicable promotion rules. | Working blackboard state, certificates, reviews, or Human Steward intent alone do not create canonical mathematical support. |
| Protected dependency layer | A protected programme dependency/evidence state whose exact status and provenance are preserved across migration. | Protected dependency status is not automatically theorem certification, formal replay status, or canonical claim status. |
| Cross-domain bridge relation | A typed, evidence-bearing relation connecting scoped domain programme subgraphs. | Similar names, equivalence proposals, or evidence in one domain do not implicitly transfer certification or canonical status into another. |
| Working blackboard state | Reducer-admitted MATH-CORE state used to coordinate live reasoning at an exact checkpoint. | Does not itself constitute canonical claim promotion or certification. |
| Assurance-bounded search constraint | A learned operational constraint whose permissible effect is bounded by the assurance class of its supporting conflict or evidence. | Remains search-only unless separately stated, supported, and promoted; heuristic evidence cannot justify hard pruning. |
| `MATHEMATICAL` blocker | A blocker arising from an unresolved mathematical statement, construction, counterexample, or argument. | Does not include formalization, governance/evidence, or runtime/tooling failure. |
| `FORMALIZATION` blocker | A blocker arising from formal statement, proof-assistant encoding, kernel replay, or formal-environment obligations. | Does not imply the underlying mathematics is false or unresolved. |
| `GOVERNANCE_EVIDENCE` blocker | A blocker arising from missing authority, provenance, review, exact-head evidence, or governed support. | Does not imply a mathematical or formal proof defect. |
| `EXECUTION_INFRASTRUCTURE` blocker | A blocker arising from runtime, tooling, controller, connector, CI, resource, or execution-path failure. | Must not be mislabeled as a mathematical or formalization blocker. |
