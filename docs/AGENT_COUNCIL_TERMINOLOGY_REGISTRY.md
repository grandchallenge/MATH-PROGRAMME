...
| Campaign promotion register | `docs/CAMPAIGN_PROMOTION_REGISTER.md`, the current record of repository-review and merge conditions discharged after an immutable campaign artifact froze its pre-merge wording. | Records documentary disposition only; it is not a theorem ledger, certificate register, novelty record, or replacement for the integrated WP00 artifact. |
...
The decision-record storage, schema-bound review, lifecycle-status, and disposition terms were normalized by `ADR-0007`. Programme-domain, public-coverage, execution-layer, and public-status terms are governed by `ADR-0008`. Governed-campaign discovery and the promotion-register boundary are governed by `ADR-0009`.
