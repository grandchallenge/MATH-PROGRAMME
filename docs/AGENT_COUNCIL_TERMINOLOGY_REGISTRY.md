...
| Public domain landing page | A concise MkDocs orientation page under `docs/domains/` that routes readers to the canonical master plan and states the public claim boundary. | Not a theorem ledger, proof artifact, source authority, or substitute for the canonical campaign dossier. |
| Documentation coverage contract | The CI-enforced agreement among `DOMAIN_REGISTRY.yaml`, public domain pages, MkDocs navigation, canonical master plans, ADRs, edition markers, historical notices, and authority pointers. | Build success alone is insufficient; semantic omissions and split authority are also rejected. |
...
The decision-record storage, schema-bound review, lifecycle-status, and disposition terms were normalized by `ADR-0007`. Programme-domain, public-coverage, execution-layer, and public-status terms are governed by `ADR-0008`.