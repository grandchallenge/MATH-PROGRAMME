...
| Decision record | A stable record of a governance or integration choice, its context, alternatives, consequences, and affected artifacts. | Not a proof and not a substitute for source provenance. |
...
| Cross-document consistency | A recorded comparison of an artifact against relevant theorem spines, claim ledgers, Work Packages, schemas, documentation, and implementation artifacts. | Consistency does not establish correctness; it establishes that known representations do not silently conflict. |
