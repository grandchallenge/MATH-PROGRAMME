(() => {
  ...
  const styleHref = '../../stylesheets/bsd-exact-plates.css';
  if (!document.querySelector(`link[data-bsd-exact-plate-layout="${styleHref}"]`)) {
    const link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = styleHref;
    link.dataset.bsdExactPlateLayout = styleHref;
    document.head.appendChild(link);
  }
  reader.classList.add('bsd-exact-landscape');
  ...
  const plateActivations = {
    ...
    '../../assets/documentaries/bsd/plate_overture.svg': {
      src: '../../assets/visual_pedagogy/bsd_exact/plate_04_strong_bsd_ledger.svg?v=bsd-math-v7-20260917',
  ...
  reader.querySelectorAll('.monograph-plate img').forEach((image) => {
    ...
    image.width = activation.width;
    image.height = activation.height;
    image.dataset.visualPedagogyActivation = 'bsd-exact-object';
    image.dataset.visualPedagogyGeometry = 'editorial-landscape-3x2';
