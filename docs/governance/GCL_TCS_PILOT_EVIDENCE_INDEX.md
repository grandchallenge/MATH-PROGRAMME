...
**Baseline protected main:** `f7d01800ff454c55d3250bc251c30c8a2b8e2886`  
**Status:** non-authoritative discovery surface

## Purpose

This file is a shallow navigation surface for the institutionalization pilot. It points to protected or otherwise governed records without copying their authority into this index.
...
## Controlling operating doctrine
...
## GHOS terminal evidence and bounded GCL-TCS pilot
...
## MATH-CORE and trusted-acceptance boundary

Mathematical pilots must preserve the protected architecture:
...
## Version-1.0 readiness baseline

This table is an observational baseline for issue `#788`; it is not a promotion decision.

| Criterion | Baseline | Current evidence/gap |
|---:|---|---|
| 1 | `PARTIAL` | Normative text, policy manifest, and schema exist; a full agreement audit is not yet evidenced here. |
| 2 | `UNSATISFIED` | Review roles exist, but an approved owner/review-role map for every profile has not been established by the inspected protected evidence. |
| 3 | `PARTIAL` | Mandatory schema fields are machine-readable and real packages validate against them; complete standard-wide validation-test coverage remains to be established. |
| 4 | `UNSATISFIED` | Fail-closed exception rules are normative; dedicated exception-workflow failure tests have not been established by the inspected evidence. |
| 5 | `PARTIAL` | The admitted GHOS pilot binds G8/G9 records to immutable revisions; broader acceptance evidence remains to be assembled. |
| 6 | `PARTIAL` | Candidate/admitted/authoritative status vocabulary exists; the complete candidate-versus-authoritative source-record distinction remains to be demonstrated against the criterion. |
| 7 | `UNSATISFIED` | Required cross-surface orphan detection has not yet been demonstrated. |
| 8 | `PARTIAL` | A real operational/governance GHOS pilot is admitted. Six-class mathematical, experimental/computational, software, operational, governance, and public coverage is incomplete. |
| 9 | `PARTIAL` | The GHOS pilot records defects, negative evidence, operator/reviewer burden, and recurring failure modes; explicit false-positive, false-negative, and unresolved-ambiguity coverage remains incomplete. |
| 10 | `OUTSIDE_CURRENT_TRANCHE` | Referee approval is a version-1.0 promotion gate. No promotion is requested by this operation. |

Readiness can improve only from exact evidence. Do not convert a `PARTIAL` row to `SATISFIED` because a similar control exists elsewhere.

## Successor work routing

The next bounded workstreams are deliberately separate:

1. **Discovery:** keep evidence shallow and pointer-based without editing terminal source objects for navigation.
2. **GHOS sentinel:** compare live material identities with governed dependencies and route changes; detect only by default.
3. **Heterogeneous pilots:** use real artifacts across all required classes; mathematical pilots must use MATH-CORE as coordination/provenance input where applicable without creating a parallel claim-state system.
4. **Measurement:** record defects, false positives/negatives, burden, ambiguities, exceptions, stale/orphan references, prevented invalid transitions, and unnecessary friction.
5. **Successor evaluation:** retain, simplify, automate, narrow, strengthen, revise, or continue the candidate pilot from evidence. Version-1.0 promotion is not an automatic outcome.

## Revalidation rule

A pointer update is routine when it exposes an unchanged governed identity. Revalidation escalates only when the material object itself changes: relevant workflow/controller/protection state, GCL-TCS normative or authority bytes, mathematical/certification/canonical state, cross-domain authority bridges, C04-C07 capability, constitutional policy, security-sensitive controls, or reserved external claim/publication authority.
