...
CHANNELS = {"n1": ("TN1",), "n2": ("TN2",), "n3": ("TN3",), "k1": ("SK", "AK", "LKK", "LLK")}
...
def specialize_rat(rat: rc.Rat, ko: int | None, lo: int | None) -> rc.Rat:
    ...
    for sig, coeff in rat.items():
        c, parts, dead = coeff, [], False
        for factor, exp in sig:
            if len(factor) == 5 and factor[0] == pcl.PINV_TAG:
                _,a,b,cv,d = factor
                ordinary = (a,b,cv,d)
                off = ko if ordinary[:3] == (1,-1,0) else lo if ordinary[:3] == (1,0,-1) else "bad"
                if off == "bad":
...
def mirror_factor(f):
    if len(f) == 5 and f[0] == pcl.PINV_TAG:
        tag,a,b,c,d = f
        return (tag,a,c,b,d)
    a,b,c,d = f
    return (a,c,b,d)
...
def mirror_rat(rat: rc.Rat) -> rc.Rat:
    ...
    for sig,c in rat.items():
        key = tuple(sorted((mirror_factor(f),e) for f,e in sig))
        out[key] = out.get(key,Q(0)) + c
        if not out[key]:
            del out[key]
    return out
...
def expected_rank(pl: pcl.Layer, channel_name: str, block: str) -> tuple[int,int]:
    reps, scalars, rows = set(BLOCKS[block]), CHANNELS[channel_name], []
