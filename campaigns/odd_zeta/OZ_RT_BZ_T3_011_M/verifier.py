...
def _protected_k_rows(pair, endpoint, uid, orientation, strata, bank, witness_index, forward, pcache, vcache):
    left, right = pair
    _axis, spectator_channel = _spectator(pair)
    indexes = _indexes(endpoint, uid, pair, strata, bank, pcache, vcache)
    ledgers = {
        "ScSd": k.trivariate_moment_ledger(witness_index, indexes["ScSd"], forward[left], "x1", forward[right], "x1", forward[spectator_channel]),
        "Sc": k.trivariate_moment_ledger(witness_index, indexes["Sc"], forward[left], "x1", forward[right], "x0", forward[spectator_channel]),
        "Sd": k.trivariate_moment_ledger(witness_index, indexes["Sd"], forward[left], "x0", forward[right], "x1", forward[spectator_channel]),
        "I": k.trivariate_moment_ledger(witness_index, indexes["I"], forward[left], "x0", forward[right], "x0", forward[spectator_channel]),
    }
    domain = set()
...
def _record_summary(pair, endpoint, uid, orientation, strata, bank, witness_index, forward, inverse, pcache, vcache):
    ...
    j_boundary, *_rest_j = _build_ledgers(
        pair,
        endpoint,
        uid,
        orientation,
    ...
    j_domain = set().union(*(value[0] for value in j_boundary.values()))
    j_selected = {(r, s, 0) for r, s, t in j_domain if r >= 1 and s >= 1 and t == 0}
    j_here = [[r, s, num, den] for r, s, _t, num, den in _rows(j_boundary, j_selected)]
    j_protected = _protected_j_rows(
        pair,
        endpoint,
        uid,
        orientation,
    ...
    k_boundary, *_rest_k = _build_ledgers(
        pair,
        endpoint,
        uid,
        orientation,
        ...
        vcache,
        lower,
        "reciprocal_degree_zero_K_boundary",
    )
    k_domain = set().union(*(value[0] for value in k_boundary.values()))
    if orientation == "negative_left_positive_right":
    ...
    k_here = _rows(k_boundary, k_selected)
    k_protected = _protected_k_rows(
        pair,
        endpoint,
        uid,
        orientation,
        ...
        for orientation_index, orientation in enumerate(producer.ORIENTATIONS):
            for endpoint_index, endpoint in enumerate(pair):
                ...
                for candidate_index, uid in enumerate(bank["candidates"]):
                    ...
                    if not summary["j_match"]:
                        first_ambiguity = record
                        independent_terminal = producer.AMBIGUITY_TERMINAL
                        stop = True
                        break
                    if not record.get("t_zero_semantics_exactly_match_J"):
                        raise AssertionError("producer J boundary claim disagrees with independent replay")
                    if not summary["k_match"]:
                        first_ambiguity = record
                        independent_terminal = producer.AMBIGUITY_TERMINAL
                        stop = True
                        break
                    if not record.get("reciprocal_degree_zero_semantics_exactly_match_K"):
                        raise AssertionError("producer K boundary claim disagrees with independent replay")
