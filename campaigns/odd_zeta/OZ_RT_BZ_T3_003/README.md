...
This package is the first executable operation after `OZ-RT-BZ-T3-002`. It does **not** prove or refute T3. It changes the representation and records the first exact search frontier in that new representation.
...
For an `l`-fibre sum, a nontrivial creative telescoper shifts an external parameter. The first experiment therefore searches the order-2 `k`-shift relation

`sum_{j=0}^2 a_j(n,k) F(n,k+j,l) = Delta_l(F(n,k,l) q(n,k,l))`.
...
The first certificate denominator is analytically locked to the denominator factors of the `l`-shift ratio:

`D = (l+1)^3(k+l+1)`.

## Exact bounded order-2 search

The producer searches five stages. The total degree of each `a_j(n,k)` runs from 0 through 4; the numerator degree of `q(n,k,l)` runs from 2 through 6; the largest retained sample reaches `n=11`.
...
The strongest stage has 432 equations, 129 unknowns, rank 129, and nullity 0. Therefore

`UNDEFORMED_PARENT_ORDER2_K_SHIFT_DSHIFT_DENOM_ADEG_LE_4_QDEG_LE_6`

is exactly inconsistent. This is not evidence that T3 is false.

## Independent verifier

`verify.py` does not import the producer or the parameter-lift implementation. It independently expands normalized Pochhammer products as exact polynomials, converts ordinary derivatives to logarithmic cumulants, reconstructs the locked harmonic letters and nested auxiliary sums, reconstructs the order-2 matrices in a different monomial order, and performs an independent modular elimination.

## Next route

The next mathematically distinct search is:

1. parameter-dependent order-2 telescoper coefficients and certificates;
2. retain the auxiliary `t` dimension required by `ES/U` through the telescoping system;
3. if order 2 remains inconsistent, advance to orders 3 and 4;
4. then test the structured symmetric two-dimensional divergence route;
5. then exact recurrence identification;
6. reserve integral or coefficient-extraction representations for the next representation change.
