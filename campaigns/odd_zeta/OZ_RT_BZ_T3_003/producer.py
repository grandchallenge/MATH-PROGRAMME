...
def row(nv,kv,lv,ad,qd):
    out=[]; ma=mon2(ad); mq=mon3(qd)
    r0=rk(nv,kv,lv); r1=rk(nv,kv+1,lv)
    for ratio in (Q(1),r0,r0*r1):
...
def matrix(ad,qd,nmax):
    ...
    for nv in range(4,nmax+1):
        for kv in range(nv-1):
            for lv in range(nv): rows.append(row(nv,kv,lv,ad,qd))
    return rows
...
def main():
    ...
    for ad,qd,nmax in STAGES:
        rows=matrix(ad,qd,nmax); unknowns=3*len(mon2(ad))+len(mon3(qd)); rank=rank_mod(rows)
        stages.append({'route_id':f'ORDER2_KSHIFT_ADEG_{ad}_QDEG_{qd}','order':2,'external_shift':'k','summation_difference':'l','a_total_degree':ad,'q_numerator_total_degree':qd,'q_denominator':'(l+1)^3*(k+l+1)','n_min':4,'n_max':nmax,'equations':len(rows),'unknowns':unknowns,'rank':rank,'nullity':unknowns-rank,'rank_certificate':'FULL_COLUMN_RANK_MOD_P_IMPLIES_FULL_COLUMN_RANK_OVER_Q','classification':'INCONSISTENT_ANSATZ' if rank==unknowns else 'CANDIDATE_SPACE_REMAINS'})
    result={'schema_version':'1.0.0','operation':'OZ-RT-BZ-T3-003','fixture':'PARAMETER_LIFT_ORDER2_001','parameter_lift':lift_info,'parent_shift_ratio_checks':ratio_checks,'search_class':'UNDEFORMED_HYPERGEOMETRIC_PARENT_ORDER2_K_SHIFT_WITH_L_CERTIFICATE','relation':'sum_{j=0}^2 a_j(n,k) F(n,k+j,l) = Delta_l(F(n,k,l)*q(n,k,l)) at lift parameters zero','denominator_derivation':'q denominator is locked to the exact denominator factors of F(n,k,l+1)/F(n,k,l)','modular_rank_certificate':{'prime':P,'denominator_condition':'all exact matrix-entry denominators are coprime to p','implication':'full column rank modulo p exhibits a nonzero maximal minor modulo p, hence the corresponding rational minor is nonzero'},'stages':stages,'strongest_frontier':stages[-1],'terminal':'NO_ORDER2_CERTIFICATE_IN_BOUNDED_UNDEFORMED_PARENT_CLASS','next_distinct_route':'PARAMETER_DEPENDENT_ORDER2_WITH_AUXILIARY_T_DIMENSION_THEN_ORDER3_ORDER4','proof_effect':'NONE','promotion_effect':'NONE'}
